bugslive = {}
function bugslive:register_mob(name, def)
	minetest.register_entity(name, {
		...
		set_animation = function(self, type)
			...
			elseif type == "walk" and self.animation.current ~= "walk"  then
				...
				elseif type == "other" and self.animation.current ~= "other"  then
				if
					self.animation.other_start
					and self.animation.other_end
					and self.animation.speed_normal
				then
					self.object:set_animation(
						{x=self.animation.other_start,y=self.animation.other_end},
						self.animation.speed_normal, 0
					)
					self.animation.current = "other"
				end
		...
		--spanch
			...
			if self.state == "stand" then
				...
				if not self.set_velocity(self, 0) then
				self.set_velocity(self, 0)
				end
				if not self.set_animation(self, "stand") then
				...
				if math.random(1, 100) <= 10 then
					self.set_velocity(self, self.walk_velocity)
					self.state = "walk"
					if not self.set_animation(self, "walk") then
					self.set_animation(self, "walk")
					end
				end
				if self.type == "kolya" then
				
				if math.random(1, 100) <= 5 then
				    if not self.set_animation(self, "other") then
					self.set_animation(self, "other")
					end
				end
...
bugslive.spawning_mobs = {}
function bugslive:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, spawn_func)
	bugslive.spawning_mobs[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if not bugslive.spawning_mobs[name] then
				return
			end
			pos.y = pos.y+1
			...
			if minetest.setting_getbool("display_mob_spawn") then
				minetest.chat_send_all("[bugslive] Add "..name.." at "..minetest.pos_to_string(pos))
			end
...
function bugslive:register_arrow(name, def)
	minetest.register_entity(name, {
		physical = false,
		visual = def.visual,
		visual_size = def.visual_size,
